...
def format_response(text):
    ...
    def repl_table(m):
        ...
        html = '<table><thead><tr>' + ''.join(f'<th>{h}</th>' for h in headers) + '</tr></thead>'
        if rows:
            html += '<tbody>'
            for r in rows:
    ...
    def repl_ul(match):
        ...
        lis = ''.join('<li>' + re.sub(r'^\s*[-+*]\s+', '', item) + '</li>' for item in items)
        return '<ul>' + lis + '</ul>'
    ...
    def repl_ol(match):
        ...
        lis = ''.join('<li>' + re.sub(r'^\s*\d+\.\s+', '', item) + '</li>' for item in items)
        return '<ol>' + lis + '</ol>'
    ...
    text = re.sub(
        r'!\[([^\]]+)\]\[([^\]]+)\]',
        lambda m: "<img src='{}' alt='{}'>".format(ref_images.get(m.group(2), ''), m.group(1)),
        text
    )
    # Inline links
    ...
    text = re.sub(
        r'\[([^\]]+)\]\[([^\]]+)\]',
        lambda m: "<a href='{}'>{}</a>".format(ref_links.get(m.group(2), ''), m.group(1)),
        text
    )
